...
bool LL_ForwardIter();
bool LL_BackwardIter();
bool BST_inorderIter();
bool BST_preorderIter();
bool BST_postorderIter();
bool BST_breadthiter();

FunctionPointer test_functions[] = { Node_empty_test , Node_not_empty_test , LL_canon 
									, LL_extracterino , LL_ins_before, LL_ins_after, LL_ForwardIter
									, LL_BackwardIter , BST_inorderIter,  BST_preorderIter, BST_postorderIter
									, BST_breadthiter };
...
bool LL_ForwardIter() 
{
	...
	for (int i = 0; i < 20; i++)
		ints.Append(i);
	LL<int>::ForwardsIterator iter(ints);
	for (int i = 0; i < 20; i++) 
	{
		if (iter.GetCurrent() != (ints.First())) 
		{
			pass = false;
		}
		iter.MoveNext();
		ints.Extract(ints.First());
	}
	for (int i = 10; i < 20; i++)
		ints.Append(i);
	iter.Reset();
...
bool LL_BackwardIter() 
{
	...
	for (int i = 0; i < 20; i++)
		ints.Append(i);
	LL<int>::BackwardsIterator iter(ints);
	for (int i = 0; i < 20; i++)
	{
		if (iter.GetCurrent() != (ints.Last())) 
		{
			pass = false;
		}
		iter.MoveNext();
		ints.Extract(ints.Last());
	}
	for (int i = 10; i < 20; i++)
		ints.Append(i);
	iter.Reset();
...
bool BST_inorderIter() 
{
	...
	ints.Insert(25);
	ints.Insert(22);
	BST<int>::InOrderIterator iter(ints);
	iter.Reset();
	int intsA[] = { 11, 12, 18, 20, 22, 25 };
	if (iter.isDone())
		pass = false;
	for (int i = 0; i < 6; i++) 
	{
		if (iter.GetCurrent() != intsA[i])
			pass = false;
		iter.MoveNext();
	}
	if (!iter.isDone())
...
bool BST_preorderIter() 
{
	...
	ints.Insert(25);
	ints.Insert(22);
	BST<int>::PreOrderIterator iter(ints);
	iter.Reset();
	int intsA[] = { 20, 12, 11, 18, 25, 22 };
	if (iter.isDone())
		pass = false;
	for (int i = 0; i < 6; i++) 
	{
...
bool BST_postorderIter()
{
	...
	ints.Insert(25);
	ints.Insert(22);
	BST<int>::PostOrderIterator iter(ints);
	iter.Reset();
	int intsA[] = { 11, 18, 12, 22, 25, 20 };
	if (iter.isDone())
		pass = false;
	for (int i = 0; i < 6; i++) 
	{
...
bool BST_breadthiter()
{
	...
	ints.Insert(25);
	ints.Insert(22);
	BST<int>::BreadthIterator iter(ints);
	iter.Reset();
	int intsA[] = { 20, 12, 25, 11, 18, 22 };
	if (iter.isDone())
		pass = false;
	for (int i = 0; i < 6; i++) 
	{
